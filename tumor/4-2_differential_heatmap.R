...
# Keep only significant exons and remove inf
diffexp_mean[(is_sig>=0.05)|(is.na(is_sig))] = NA
diffexp_mean[is.infinite(diffexp_mean)] = NA
...
RAA_pca = read.csv("results/RAA_pca.txt", row.names = 1, sep = "\t")
breaks = c(min(diffexp_mean,na.rm=T),seq(-1,1,length.out=254),max(diffexp_mean,na.rm=T))
diffexp_mean=diffexp_mean[order(RAA_pca[rownames(diffexp_mean),1],decreasing = T),order(colSums(is.na(diffexp_mean)))]
heatmap.2(diffexp_mean,
          Rowv=F, Colv=F, col = bluered(255), symm=T, margins=c(9,15), trace="none",
          ylab="delta PSI", xlab="Cancer Types", na.color="grey",breaks = breaks)
