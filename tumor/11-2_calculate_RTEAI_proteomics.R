...
trna = trna[,(tissues %in% c("BRCA","COAD","READ"))]
...
for (type in names(cancer_types)){
  ...
  TAIs = matrix(NaN,ncol = ncol(temptrna), nrow = ncol(codon));
  colnames(TAIs) = colnames(temptrna); rownames(TAIs) = colnames(codon)
  ...
  TAIs[,] = sample.tai
  write.csv(TAIs,sprintf("results/RTEAI_CUproteomics%s_sqrt.csv",type))
}