...
for (t in names(cancer_types)){
  ...
  for (type in types){
    ...
    expr_samples = substr(colnames(rawvalues),1,15)
...
write.csv(summary_table, sprintf("results/differentialMeth_TvsH_twosided.csv",type))
...
summary_table$pval = pvals
summary_table$pval_adj = p.adjust(pvals,method="fdr")
write.csv(summary_table, sprintf("results/differentialCNA_TvsH_twosided.csv",type))
