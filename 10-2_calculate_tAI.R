...
# TAIs
TAIs = read.csv("results/tAI_CUgenomic.csv", row.names = 1)
...
for (s in tissues_unique){
  # TAIs
  idxtissue = (tissues==s)
  TAIs = read.csv(sprintf("/home/xhernandez/Downloads/tAI_genomic/%s_tAI_CUgenomic.csv",s), row.names = 1)
  # Rename samples based on clustering
  colnames(TAIs) = samples
  
  # Calculate CU
  weightedTAI_trna[,idxtissue] = (as.matrix(t(matched_genexp))%*%as.matrix(TAIs[,idxtissue]))/colSums(matched_genexp)
  weightedTAI_CU[idxtissue,] = (as.matrix(t(matched_genexp[,idxtissue]))%*%as.matrix(TAIs))/colSums(matched_genexp[,idxtissue])
}

# Calculate weighted tAI
write.csv(weightedTAI_trna,"results/TStrna_weighted_tAI_trnacentric.csv")
write.csv(weightedTAI_CU,"results/TStrna_weighted_tAI_CUcentric.csv")
