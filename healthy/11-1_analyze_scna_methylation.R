...
## Analyze methylation
correlations = data.frame(row.names = rownames(anticodon))
for (type in names(cancer_types)){
  ...
  correlations[acods,type] = data.frame(sapply(acods,function(x) cor(anticodon[x,merged_trna],
                                                     values[x,merged_value_idx],method = "spearman",use="na.or.complete")))
}
write.csv(correlations,"results/trnaH_methylation_corr.csv")
...
write.csv(correlations,"results/trnaH_CNA_corr.csv")
