...
colnames(anticodon) = gsub("KICH","kidney",colnames(anticodon))
colnames(anticodon) = gsub("KIRP","kidney",colnames(anticodon))
colnames(anticodon) = gsub("KIRC","kidney",colnames(anticodon))
colnames(anticodon) = gsub("LUAD","lung",colnames(anticodon))
colnames(anticodon) = gsub("LUSC","lung",colnames(anticodon))
colnames(anticodon) = gsub("COAD","colorectal",colnames(anticodon))
colnames(anticodon) = gsub("READ","colorectal",colnames(anticodon))
colnames(anticodon) = gsub("CHOL","liver",colnames(anticodon))
colnames(anticodon) = gsub("LIHC","liver",colnames(anticodon))
colnames(anticodon) = gsub("UCEC","uterus",colnames(anticodon))
colnames(anticodon) = gsub("CESC","uterus",colnames(anticodon))
...
expr_bytissue = data.frame(row.names = rownames(anticodon))
...
for (t in tissues_unique){
  ...
  tissue_mean = rowMeans(anticodon[,tissue_bool],na.rm=T)
  expr_bytissue[,t] = tissue_mean
}

# Calculate tAI per tissue
...
for (t in tissues_unique){
  ...
  # Remove nonspecific tRNAs
  trna_specific = as.numeric(rownames(expr_bytissue) %in% rownames(expr_bytissue)[order(expr_bytissue[,t])[1:20]])*runif(64,1,20)
  temp = get.ws(tRNA=trna_specific, s=initial_s, sking=0)
  codon_nonspecific = (duplicated(temp)|duplicated(temp,fromLast = T))
  # Calculate tAI
  for (sample in colnames(anticodon)){
    ...
    # Calculate tAI for all CUs. Consider only codons with tissue-specific tRNAs
    sample.tai <- get.tai(t(codon[!codon_nonspecific,]), sample.ws[!codon_nonspecific])
    TAIs[,sample] = sample.tai
  }
  write.csv(TAIs,sprintf("/home/xhernandez/Downloads/tAI_genomic/%sbottom_tAI_CUgenomic.csv",t))
}
